...
The repository already ships with a generated `.mif`. To use your own image, run the MATLAB converter script:

* **Test Image:** By default, the script is configured to test with the image named `PERROQUE_256_256.bmp`.
* **Custom Image:** To convert a different image, update the filename variable directly inside the MATLAB script, and ensure the new **256x256 image** is placed in the same directory.
* **Quartus Integration:** Running the script will generate a `memory_init.mif` file. You must then load this file into your **ROM: 1-PORT** IP core within Intel Quartus to initialize the memory.
